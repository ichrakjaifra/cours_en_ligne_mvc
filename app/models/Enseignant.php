...
class Enseignant extends Utilisateur {
  ...
  public static function getAllEnseignants() {
      $db = Database::getInstance()->getConnection();
      try {
          $stmt = $db->prepare("SELECT u.* FROM utilisateurs u WHERE u.role_id = 2");
          $stmt->execute();
          $enseignants = [];
  ...
  // Méthode pour récupérer le nombre de catégories enseignées
  public function getNombreCategories() {
      $db = Database::getInstance()->getConnection();
      try {
          $stmt = $db->prepare("SELECT COUNT(DISTINCT c.categorie_id) AS nombre_categories FROM courses c WHERE c.enseignant_id = ?");
          $stmt->execute([$this->getId()]);
